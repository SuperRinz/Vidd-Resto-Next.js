    ...
    <div className="p-8 bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row gap-8 items-start">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold mb-4 text-black">Daftar Menu</h1>
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full font-semibold text-sm border transition ${
                  activeCategory === cat.id
                    ...
                    : "bg-white text-black border-gray-300 hover:border-[#0d9488]"
                }`}
              >
                {cat.label}
              </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredMenus.map((item) => (
              <div key={item.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                ...
                <p className="text-[#0d9488] font-bold mb-3">Rp {item.price.toLocaleString()}</p>
        ...
        <div className="w-full md:w-80 bg-white p-6 rounded-2xl shadow-xl border border-gray-200 sticky top-8">
          ...
          <input 
            ...
            className="w-full p-2 mb-4 border rounded-lg focus:ring-2 focus:ring-[#0d9488] outline-none text-black"
            value={namaPembeli}
          ...
          <div className="border-t pt-4">
            <div className="flex justify-between font-bold text-lg mb-4">
              ...
              <span className="text-[#0d9488]">Rp {totalBayar.toLocaleString()}</span>
            </div>
            <button 
              onClick={handleCheckout}
              disabled={totalBayar === 0}
              className="w-full bg-[#0d9488] text-white py-3 rounded-xl font-bold hover:bg-[#0f766e] disabled:bg-gray-200 transition"
            >
              Konfirmasi Pesanan
            </button>
          </div>
