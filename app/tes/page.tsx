...
useEffect(() => {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      setKeranjang({});
      setNamaPembeli("");
      fetchInitialData(); // Refresh daftar order
    } catch (err: any) {
        ...
        <div className="flex-1">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredMenus.map((item) => (
              <div key={item.id} className="bg-white p-3 sm:p-4 rounded-xl shadow-sm border border-gray-100">
                <img src={item.image_url} className="w-full h-24 sm:h-32 object-cover rounded-md mb-2" />
                <h3 className="font-bold text-black text-sm sm:text-base">{item.item_name}</h3>
                <p className="text-orange-600 font-bold mb-2 text-sm sm:text-base">Rp {item.price.toLocaleString()}</p>
